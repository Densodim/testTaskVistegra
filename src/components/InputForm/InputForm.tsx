...
function InputForm() {
	...
	const handleChange = async (key: SizeConfigType, value: number) => {
	...
	if (!materialsIsLoading) {
		return (
			<>
				...
				<div className={style.inputSection}>
					...
					<AComponentUsingInput
						type={"number"}
						getSizeConfig={getSizeConfig("width")}
						value={form.width}
					...
					<AComponentUsingInput
						type={"number"}
						getSizeConfig={getSizeConfig("length")}
						value={form.length}
		...
		return <>{materialsError || configError}</>
	}
}
