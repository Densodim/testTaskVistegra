...
function Results() {
	...
	const selectFix = config
		?.filter((el) => el.type === "fix")
		.find((el) => el.key === selectOptions?.choiceOfMaterial)
	const selectFrame = config
		?.filter((el) => el.type === "frame")
		.find((el) => el.key === selectOptions?.choiceOfFrame)
				...
				<table className={styles.resultTable}>
					...
					<tbody>
						...
						<tr>
							<td>Саморез</td>
							<td>
								{selectFix?.name} - {selectFix?.value}
							</td>
							<td>{totalScrews}</td>
						</tr>
					</tbody>
